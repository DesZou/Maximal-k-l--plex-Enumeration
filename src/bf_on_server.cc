...
void init() {
	...
	for (u32 i = 0; i < g.vrt_num; ++i) {
		b_edges[i].resize(g.vrt_num);
		b_edges_r[i].resize(g.vrt_num);
		for (auto& v : g.edges[i]) b_edges[i][v] = true;
		for (auto& v : g.edges_r[i]) b_edges_r[i][v] = true;
	}
}
